...
| `GET /v2/conversations/:id/messages`                                          | Authorized, reverse-chronological history pagination rendered oldest-first. `contextPack=true` selects the bounded, anchored agent context operation. |
...
An eligible Hermes message requests a context pack only after its self-message, author
...
The CLI validates and renders this pack, then emits an adapter-protocol-1 envelope. Python
verifies the envelope, binds the result to its authorized conversation and trigger, and adds
Hermes routing policy. It does not maintain TypeScript entity or Unicode schema copies.
The mandatory Hermes gate runs Ruff, mypy and real CLI-to-Python fixtures.
